...
function requirements(lastFresh?: number): string {
  const lines = [
    "- 给 4 到 6 首，不要少给。",
    // 说成「至少 2 首」时，模型把它当开关：要么一首库外都没有，" +
    // 要么整组全是陌生器乐（实测连续四场是 1 / 4 / 1 / 3）。
    // 说成配比它才会去**编排**这一组，而不是二选一。
    "- 这一组必须是**混的**，一半一半：",
    "    · 一半来自他榜单上的熟面孔 —— 那是他愿意按下播放的理由；",
    "    · 另一半的艺人**不能**出现在上面任何一张榜单里 —— 那是电台存在的理由。" +
      "照着他听了几百次的艺人推，他自己就会放，不需要你。",
    "  库外那半至少 2 首。全是熟人不合格，全是陌生人同样不合格。",
    "- 但陌生 ≠ 编造：拿不准是否真实存在的，换一首你确信存在的陌生歌。",
    "- 在 reason 里点明哪几首是库外的。",
  ];
  // 每一轮都把上一轮的实际数字报回去，达标与否都报。
  //
  // 只在不达标时提醒会让它震荡：被点名就补上，达标后没人提醒就又松回去
  // （实测连续三场是 1 → 3 → 0）。模型自己数不清「这个名字在不在那张榜上」，
  // 所以这个数字是它唯一的反馈来源，断掉一轮它就失去准绳。
  if (typeof lastFresh === "number") {
    lines.push(
      lastFresh >= 2
        ? `- 上一轮你给出了 ${lastFresh} 首真正库外的（系统核对过）。这一轮至少同样多。`
